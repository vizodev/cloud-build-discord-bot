...
export class DiscordRepository {
  ...
  async handleMessage(
    ...
    const embedBuilder = new EmbedBuilder()
      .setAuthor(this.getTitleForEmbed({ status, projectId }))
      .setDescription(
        this.getDescriptionForEmbed({
          status,
          projectId,
          startTime,
          finishTime,
  ...
  async handleGithubEvent(
    ...
    const embedBuilder = new EmbedBuilder()
      .setAuthor({
        name: `New build available for project ${repoName}`,
    ...
    const actionRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`artifact-${artifactsUrl}`)
        .setLabel("Generate Download URL")
  ...
  getTitleForEmbed(data: { status: string; projectId: string }) {
    ...
    switch (status) {
      ...
      default:
        ...
        iconURL =
          "https://images.emojiterra.com/google/android-10/512px/23f3.png";
    }
    return { name, iconURL };
  }

  getDescriptionForEmbed(data: {
    status: string;
    projectId: string;
    startTime: string;
    finishTime: string;
    ...
    const { status, projectId, startTime, finishTime, steps, tags } = data;
    let description = `- Project ${projectId}\n`;
    switch (status) {
      case "SUCCESS":
        description += `- Start Time: ${format(
          new Date(startTime),
          "dd/MM/yyyy HH:mm"
        )}\n- Finish Time: ${format(
          new Date(finishTime),
